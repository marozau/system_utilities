	...
	namespace tests_
	{
		namespace common
		{
			namespace details
			{
				...
				{
					typedef ts_logger< logger< turn_on, flush_stream, print_prefix > > ts_logger;
					{
			...
			void ts_logger_constructor_tests()
			{
				...
				BOOST_CHECK_NO_THROW( (ts_logger< logger< false > >( str )) );
				BOOST_CHECK_NO_THROW( (ts_logger< logger< true > >( str )) );
				BOOST_CHECK_NO_THROW( (ts_logger< logger< true, false > >( str )) );
				BOOST_CHECK_NO_THROW( (ts_logger< logger< true, false, false > >( str )) );
				BOOST_CHECK_NO_THROW( (ts_logger< logger< true, true, false > >( str )) );
			}
			void ts_logger_write_tests()
			{
				{
					std::stringstream stream;
					ts_logger< logger< false > > no_logger( stream );
				...
				{
					std::stringstream stream;
					ts_logger< logger< true, false > > log( stream );
				...
				{
					std::stringstream stream;
					ts_logger< logger< true, false, false > > log( stream );
					log.write( system_utilities::common::details::message_level::note, "message" );
					const std::string stream_content = stream.str();
			...
			void ts_logger_flush_tests()
			{
				...
				ts_logger< logger<> > l( it_is_just_a_test_on_existing_flush_method );
