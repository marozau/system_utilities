...
namespace system_utilities
{
	namespace tests_
	{
		namespace common
		{
			namespace details
			{
				void spsc_queue_constructor_test_helper()
				{
					spsc_queue< size_t, 1024 > mq;
					for ( int i = 1; i < 10; i++ )
				...
				typedef spsc_queue< size_t, 1024 > spsc_queue_size_t;
				void spsc_queue_different_threads_pop_thread_helper( spsc_queue_size_t* mq, const size_t different_threads_test_size )
				{
				...
				void spsc_queue_wait_pop_test_helper( spsc_queue_size_t* mq )
				{
				...
				struct spsc_queue_many_threads_test_helper
				{
					spsc_queue_size_t mq_;

					explicit spsc_queue_many_threads_test_helper( const size_t push_threads, const size_t pop_threads, const size_t message_size )
					{
						BOOST_CHECK_EQUAL( mq_.empty(), true );
						boost::thread_group tg_push, tg_pop;
						for ( size_t i = 0; i < push_threads; ++i )
							tg_push.create_thread( boost::bind( &spsc_queue_many_threads_test_helper::pusher, this, message_size ) );
						...
						for ( size_t i = 0; i < pop_threads; ++i )
							tg_pop.create_thread( boost::bind( &spsc_queue_many_threads_test_helper::poper, this, to_pop ) );
						tg_push.join_all();
						tg_pop.join_all();
						mq_.stop_processing();
					}
				...
				void spsc_queue_wait_test_helper( details::spsc_queue_size_t* mq_, size_t* pop_iterations_ )
				{
					while ( true )
					{
						size_t* s = mq_->wait_pop();
			...
			void spsc_queue_constructor_tests()
			{
				...
				details::spsc_queue_constructor_test_helper();
			}
			...
			void spsc_queue_wait_pop_tests()
			{
				details::spsc_queue_size_t mq;
				BOOST_CHECK_EQUAL( mq.empty(), true );
				boost::thread pop = boost::thread( boost::bind( &details::spsc_queue_wait_pop_test_helper, &mq ) );
			...
			void spsc_queue_many_threads_tests()
			{
				const size_t message_amount = 1000000;
				{
					...
					details::spsc_queue_many_threads_test_helper helper( 1, 1, message_amount );
					const auto time = tt.elapsed();
					BOOST_CHECK_EQUAL( time, 100 );
				}
				{
					time_tracker< std::chrono::milliseconds > tt;
					details::spsc_queue_many_threads_test_helper helper( 1, 1, message_amount * 4 );
					const auto time = tt.elapsed();
					BOOST_CHECK_EQUAL( time, 100 );
				}
			}
			void spsc_queue_wait_tests()
			{
				details::spsc_queue_size_t mq;
				mq.restart();
				BOOST_CHECK_EQUAL( mq.empty(), true );
				...
				boost::thread poper( boost::bind( &details::spsc_queue_wait_test_helper, &mq, &pop_iterations_ ) );
				mq.wait();
				mq.stop();
				poper.join();
